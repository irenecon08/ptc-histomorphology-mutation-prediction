"""
hypothesis1_checks.py

HYPOTHESIS 1: the negative attention-immune correlation is a mechanical
...
Four checks, per task (multiclass, ret_binary):
...
def check_1a(df):
    ...
    print(f"      -> more moderate (e.g. -0.4 to -0.6) suggests real slack exists.")
    return r
...
def check_1b(df):
    ...
    print(f"  1b. mean 'other' (unclassified) nuclei share per tile: {mean_other:.1%}")
    print(f"      -> near 0% means epithelial+immune ~fully occupy each tile "
          f"(tighter compositional trap).")
...
def check_1c(df):
    ...
    print(f"      -> ratio near 1.0: relationship survives using count, weakens the "
          f"pure-density-artifact explanation.")
    print(f"      -> ratio well below 1.0: relationship is much weaker with count, "
          f"supports the density-artifact explanation.")
...
def check_1d(df, n_bins=N_BINS):
    ...
    if len(rdf):
        ...
        print(f"      -> if within-bin correlations remain consistently negative "
              f"(even if weaker than the overall correlation), the relationship is "
              f"not fully explained by epithelium varying between tiles.")
        print(f"      -> if within-bin correlations are near zero or inconsistent "
              f"in sign, epithelium alone plausibly explains the overall pattern.")
    return rdf
