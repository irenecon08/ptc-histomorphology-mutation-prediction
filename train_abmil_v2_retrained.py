...
# === NEW hyperparameters, selected via validation-only tuning on corrected v2 data ===
# (previously lr=1e-5, [512,256] literature default for both - now updated per task)
TASK_CONFIGS = {
    "multiclass": {"lr": 1e-4, "attn_dim": 512, "fc_dim": 512},
    "ret_binary": {"lr": 5e-5, "attn_dim": 512, "fc_dim": 256},
}
...
def run_experiment(task, num_classes, class_weights=None):
    ...
    print(f"\n{'='*60}\nTraining ABMIL v2 (RETRAINED, new hyperparameters): {task}")
    print(f"lr={cfg['lr']}, attn_dim={cfg['attn_dim']}, fc_dim={cfg['fc_dim']}")
    print(f"{'='*60}")
    ...
    model = ABMIL(EMBED_DIM, cfg["attn_dim"], cfg["fc_dim"], num_classes, DROPOUT).to(DEVICE)
    if class_weights:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_auc > best_val_auc:
            ...
            torch.save(model.state_dict(), os.path.join(RESULTS_DIR, f"best_{task}_v2_retuned.pt"))
            patience_counter = 0
        else:
            patience_counter += 1
    ...
    # Load best model and evaluate on test EXACTLY ONCE - this is the official result
    model.load_state_dict(torch.load(os.path.join(RESULTS_DIR, f"best_{task}_v2_retuned.pt")))
    ...
    results = {
        "task": task, "model": "ABMIL_v2_RETRAINED",
        "hyperparameters": cfg,
        "test": {"bal_acc": test_bal_acc, "auc": test_auc, "auprc": test_auprc, "f1": test_f1},
        "history": history
    }
    with open(os.path.join(RESULTS_DIR, f"abmil_results_{task}_v2_retuned.json"), "w") as f:
...
if __name__ == "__main__":
    ...
    print("Retraining both tasks with NEW validation-selected hyperparameters (no test leakage).")
    ...
    print("\nAll ABMIL v2 RETRAINING complete!")
    print("New models saved as: best_multiclass_v2_retuned.pt, best_ret_binary_v2_retuned.pt")
    print("Original best_multiclass_v2.pt / best_ret_binary_v2.pt were NOT touched.")
