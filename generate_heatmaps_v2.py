...
OUTPUT_DIR = "/cs/student/project_msc/2025/aibh/iconstan/heatmaps_v2"
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

EMBED_DIM, ATTN_DIM, FC_DIM, DROPOUT = 1536, 512, 256, 0.0
THUMBNAIL_MAX_DIM = 1024
ALPHA = 0.45
RET_OPTIMAL_THRESHOLD = 0.09  # found via validation-set threshold search
...
def select_examples(splits_df, task, n_correct=2, n_incorrect=1):
    """Select correctly-classified and misclassified test-set examples per class."""
    ...
    for cls_idx, cls_name in enumerate(class_names):
        ...
        for patient in candidates:
            ...
            # Apply optimal threshold for RET binary task; argmax for multiclass
            if task == "ret_binary":
...
def generate_heatmap(patient, model, task, true_class, status):
    ...
    for (x, y), score in zip(coords, percentile_scores):
        # Use the CORRECT stored level_downsample (fixes the v1 coordinate bug)
    ...
    axes[1].set_title(f"Attention Heatmap (v2, corrected)\n{pred_class_str}", fontsize=9)
    axes[1].axis("off")
...
if __name__ == "__main__":
    ...
    print("=== Selecting examples: Aim a) Multiclass (v2, corrected) ===")
    ...
    print("\n=== Selecting examples: Aim b) RET binary (v2, corrected, optimal threshold) ===")
